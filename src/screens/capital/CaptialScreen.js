...
export const CapitalScreen = props => {
  ...
  function addCapital(capitals){

    let s = 0;
    for(i in capitals){

      console.log("All Amounts: " + capitals[i].Calc);
      s += capitals[i].Calc;
    }
    getSum(s);
  }
  ...
  function add(){
    navigator.navigate('CapGainLossAdd');
  }
  ...
  const getYear = async () => {
    try {
      ...
      const response = await fetch( serverAddress + '/capYear', {
        method: 'POST',
        headers: {
  ...
  const getData = async () => {
      try {
        ...
        const response = await fetch( serverAddress + '/CapitalGainLoss', {
          method: 'POST',
        ...
        if (response.ok) {
          ...
          if (responseData.result == -2) {
            ...
            console.log("Capital Gain Loss Data Retrieved");
            console.log(responseData.message);
            setList(responseData.message);
            addCapital(responseData.message);
          }
          else {
            console.log("Capital Gain Loss Data Failed to be Retrieved");
            Toast.show({
              position: 'bottom',
              type: 'error',
              text1: responseData.message
  ...
  return (
    ...
    <View style={tailwind('w-full h-min mt-24 justify-center items-center')}>
      <Text style={{fontFamily: 'Roboto-Medium', fontSize: 28, color: '#2032d4',fontWeight: '500',}}>${sum}</Text>
      </View>
  ...
  <View style={tailwind('w-full flex flex-row justify-between px-5 mt-10 pt-1')}>
      ...
      <View style={tailwind('h-3/5 mx-7 mb-20 mt-5 pb-12')}>
        <FlatList
          ...
          renderItem={({item}) => (
            <CustomRow Asset={item.Asset} Calc={item.Calc} CapitalID={item.CapitalID}/>
          )}
...
const CustomRow = ({Asset, Calc, CapitalID}) => {
  const tailwind = useTailwind();
  const navigator = useNavigation();
  function view(){
  ...
  return (
    ...
    <TouchableOpacity onPress={view}
      ...
      <View  style={tailwind('mt-5')}>
          <FontAwesomeIcon icon="fa-solid fa-arrow-trend-up"  style={tailwind('absolute w-10 h-10 ml-2')} size={36}/>
          <Text style={tailwind(' ml-12 text-xs font-semibold text-black')}>{Asset}</Text>
      <Text style={tailwind('ml-12 text-xs text-[#C6C6C6]')}> {"$" + Calc} </Text>
        </View>
    </TouchableOpacity>
    </ShadowedView>
  );
...
const BottomNav = () => {
  ...
  return (
    <View style={tailwind('absolute bottom-2 z-10 right-4 left-4 h-14 border border-gray-300 rounded-full bg-stone-900 justify-evenly flex-row items-center')}>
      ...
      <FontAwesomeIcon icon="fa-solid fa-arrow-trend-up" style={[{color: 'white'}]} size={20}/>
