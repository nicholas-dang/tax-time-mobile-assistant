...
export const DeductionScreen = props => {
  ...
  function addDeductions(deductions){

    let s = 0;
    for(i in deductions){

      console.log("All Amounts: " + deductions[i].Amount);
      s += deductions[i].Amount;
    }
    getSum(s);
  }
  ...
  function add() {
    //go back to home screen
    navigator.navigate('DeductionsExpensesAdd');
  }
  ...
  const getYear = async () => {
    try {
      ...
      const response = await fetch( serverAddress + '/dedYear', {
        method: 'POST',
        headers: {
  ...
  const getData = async () => {
      try {
        ...
        const response = await fetch( serverAddress + '/TaxDeductionsExpenses', {
          method: 'POST',
        ...
        if (response.ok) {
          ...
          if (responseData.result == -2) {
            ...
            addDeductions(responseData.message);
          }
          else {
 ...
 <View style={tailwind('w-full flex flex-row justify-between px-5 mt-10 pt-1')}>
      ...
      <View style={tailwind('h-3/5 mx-7 mb-20 mt-5 pb-12')}>
        <FlatList
          ...
          renderItem={({item}) => (
            <CustomRow Type={item.Type} Description={item.Description} ExpenseID={item.ExpenseID} Amount={item.Amount}/>
          )}
...
const CustomRow = ({Type, Description, ExpenseID, Amount}) => {
  const tailwind = useTailwind();
  const navigator = useNavigation();
    ...
    <TouchableOpacity  onPress={view}
      ...
      {/* <Image 
        ...
        style={tailwind('absolute w-12 h-12 ml-3 mt-3')}></Image> */}
          ...
          <Text style={[{fontFamily: 'Roboto-Medium', fontSize: 12, color: '#333333'},tailwind('ml-12')]}>
        {Type}
      </Text>
...
const BottomNav = () => {
  ...
  return (
    <View style={tailwind('absolute bottom-2 z-10 right-4 left-4 h-14 border border-gray-300 rounded-full bg-stone-900 justify-evenly flex-row items-center')}>
      ...
      <FontAwesomeIcon icon="fa-solid fa-file-lines" style={[{color: 'white'}]} size={20}/>
...
const emptyListMessage = () => {
  ...
  return (
    ...
    <Text>No Deductions/Expenses Recorded</Text>
    </View>
  )
};
