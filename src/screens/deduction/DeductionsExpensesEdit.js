...
export const DeductionsExpensesEdit = (props) => {
    ...
    function goBack() {
        ...
        navigator.navigate('DeductionsExpensesView', {ExpenseID:ExpenseID});
    }
    ...
    const [Type, setType] = useState(this.state.Type);
    ...
    console.log("Expenses on Edit: ExpenseID: " + ExpenseID + " Type: "  + Type + " Amount: " + Amount + " Description: " + Description + " Date: " + selectedDate);
    ...
    const deleteSubmit = async () => {
      ...
      setIsLoading(true);
        try {
          ...
          const response = await fetch( serverAddress + '/deleteExpense', { 
            method: 'POST',
          ...
          if (response.ok) {
            ...
            if (responseData.result != -1) {
              ...
              console.log("Failed to delete deduction");
              Toast.show({
                position: 'bottom',
                type: 'error',
                text1:  responseData.message
   ...
   const handleSubmit = async () => {
      ...
      setIsLoading(true);
        try {
          ...
          const response = await fetch(serverAddress + '/editExpense', { 
            method: 'POST',
            signal: controller.signal,
            headers: { 
              'Content-Type': 'application/json',
          ...
          if (response.ok) {
            ...
            if (responseData.result == -2) {
              ...
              console.log("Failed to edit deduction");
              Toast.show({
                position: 'bottom',
                type: 'error',
                text1:  responseData.message
    ...
    return (
        <SafeAreaView style={tailwind('w-full h-full bg-white')}>
            ...
            <Text style={[{fontFamily: 'Roboto-Medium', fontSize: 34, color: '#333333'}, tailwind('text-center px-4 mt-5')]}>Deductions/Expenses</Text>
