...
export const DeductionsExpensesView  = (props) => {
    ...
    const [ExpenseID] = useState(this.state.ExpenseID);
    console.log("ExpenseID on view: " + ExpenseID);
    ...
    const getData = async () => {
      setIsLoading(true);
        try {
          ...
          const response = await fetch(serverAddress + '/viewExpense', {
            method: 'POST',
          ...
          if (response.ok) {
            ...
            if (responseData.result == -2) {
              ...
              console.log("Deduction Data Retrieved");
              setType(responseData.message.Type);
              setAm(responseData.message.Amount);
              setDescription(responseData.message.Description);
              setSelectedDate(moment(responseData.message.ExpenseDate).format('DD/MM/YYYY'));
              ...
              console.log("Deduction Data Failed to be Retrieved");
              Toast.show({
                position: 'bottom',
                type: 'error',
                text1: responseData.message
    ...
    function edit(){
        navigator.navigate('DeductionsExpensesEdit', {ExpenseID:ExpenseID, Type:Type, Amount:Amount, selectedDate:selectedDate, Description:Description});
    }
    ...
    return (
        <SafeAreaView style={tailwind('w-full h-full bg-white')}>
            ...
            <View style={tailwind('px-5 mt-10')}>
                <TextInput editable={false}  style={tailwind('h-12 w-full border border-gray-300 rounded-lg pl-3 text-lg bg-disabled-gray')} placeholder='Type'>{Type}</TextInput>
            </View>

            <View style={tailwind('px-5 mt-5')}>
                <TextInput editable={false}  style={tailwind('h-12 w-full border border-gray-300 rounded-lg pl-3 text-lg bg-disabled-gray')} placeholder='Amount'>${Amount}</TextInput>
            </View>
