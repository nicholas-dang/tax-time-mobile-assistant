...
export const DonationView  = (props) => {
    ...
    console.log("DonationID on view: " + DonationID);
    ...
    const getData = async () => {
      setIsLoading(true);
        try {
          ...
          const response = await fetch( serverAddress + '/viewDonation', {
            method: 'POST',
          ...
          if (response.ok) {
            ...
            if (responseData.result == -2) {
              ...
              setOrg(responseData.message.Organization);
              setAm(responseData.message.Amount);
              setSelectedDate(moment(responseData.message.DonationDate).format('DD/MM/YYYY'));
    ...
    function edit() {
        //go to edit screen
        navigator.navigate('DonationEdit', {DonationID:DonationID, Organization:Organization, Amount:Amount, selectedDate:selectedDate});
    }
    ...
    return (
        <SafeAreaView style={tailwind('w-full h-full bg-white')}>
            ...
            <View style={tailwind('px-5 mt-10')}>
                <TextInput  editable={false}  style={tailwind('h-12 w-full border border-gray-300 rounded-lg pl-3 text-lg bg-disabled-gray')} placeholder='Organisation'>{Organization}</TextInput>
            </View>
