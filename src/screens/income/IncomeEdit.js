...
export const IncomeEdit = (props) => {
    ...
    const [Title, setTitle] = useState(this.state.Title);
    const [Amount, setAmount] = useState((this.state.Amount).toString());
    const [Description, setDescription] = useState(this.state.Description);
    ...
    console.log("Income on Edit: IncomeID: " + IncomeID + " Amount: " + Amount + " Description: " + Description + " Date: " + selectedDate);

    function goBack() {
        navigator.navigate('IncomeView', {IncomeID:IncomeID});
    }
    ...
    function TitleVerification(){
      ...
      if (Title != '') {
        ...
        setTitleVerify('Must input "Title"');
        return false;
      }
    }
    ...
    const deleteSubmit = async () => {
      ...
      setIsLoading(true);
          try {
            ...
            const response = await fetch( serverAddress + '/deleteIncome', { 
              method: 'POST',
     ...
     const handleSubmit = async () => {
      ...
      const Date = mn;
        ...
        if(Title.length == 0 || (Amount == undefined || Amount == '' || Amount == ' ' || isNaN(Amount)) || 
        ...
        setIsLoading(true);
          try {
            ...
            const response = await fetch(serverAddress + '/editIncome', { ///addincome
              method: 'POST',
              signal: controller.signal,
              headers: { 
                'Content-Type': 'application/json',
    ...
    const [fileToDeleteList,setFileToDeleteList] = useState([]);
    ...
    return (
        <SafeAreaView style={tailwind('w-full h-full bg-white')}>
            ...
            <Text style={[{fontFamily: 'Roboto-Medium', fontSize: 34, color: '#333333'}, tailwind('text-center px-4 mt-5')]}>Income</Text>

            <View style={tailwind('px-5 mt-5')}>
            <Text style={tailwind('text-red-500 text-sm ml-1')}>{TitleVerify}</Text>
                <TextInput maxLength={30} style={tailwind('h-12 w-full border border-zinc-200 rounded-lg pl-3 text-lg')} value={Title} onChangeText={setTitle} placeholder='Title' onBlur={TitleVerification}></TextInput>
            </View>
