...
export const IncomeScreen = props => {
  ...
  function addIncome(income){
    ...
    for(i in income){
      ...
      console.log("All Income: " + income[i].Amount);
      s += income[i].Amount;
    }
    getSum(s);
  }
  ...
  const getYear = async () => {
    try {
      ...
      const response = await fetch( serverAddress + '/incYear', {
        method: 'POST',
        headers: {
  ...
  const getData = async () => {
      try {
        ...
        const response = await fetch(serverAddress + '/Income', {
          method: 'POST',
        ...
        if (response.ok) {
          ...
          if (responseData.result == -2) {
            ...
            addIncome(responseData.message);
            console.log(responseData.message);
 ...
 <View style={tailwind('w-full flex flex-row justify-between px-5 mt-10 pt-1')}>
      ...
      <View style={tailwind('h-3/5 mx-7 mb-20 mt-5 pb-12')}>
        <FlatList
          ...
          renderItem={({item}) => (
            <CustomRow Title={item.Title} Amount={item.Amount} Description={item.Description} IncomeID={item.IncomeID}/>
          )}
...
const CustomRow = ({Title, Amount, Description, IncomeID}) => {
  const tailwind = useTailwind();
  const navigator = useNavigation();
  function view(){
...
const BottomNav = () => {
  ...
  return (
    <View style={tailwind('absolute bottom-2 z-10 right-4 left-4 h-14 border border-gray-300 rounded-full bg-stone-900 justify-evenly flex-row items-center')}>
      ...
      <FontAwesomeIcon icon="fa-solid fa-briefcase" style={[{color: 'white'}]} size={20}/>
